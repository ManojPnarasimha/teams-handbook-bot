"""LLM provider client — Groq default, Gemini alternate.

Provider is resolved ONCE at startup from env vars. No silent runtime fallback.
Owns the grounding + anti-injection system prompt and the no-context HR fallback.
"""
...
# Deterministic fallback shown when retrieval finds nothing.
NO_CONTEXT_MESSAGE = (
    "I couldn't find anything relevant to that in our documents. "
    "Please contact HR directly for help with this."
)

# System prompt: grounding + anti-injection + no-context contract.
...
class LLMError(Exception):
    """Raised for any recoverable LLM-side failure (already sanitised)."""


def resolve_provider() -> str:
    """Decide provider once. Explicit override wins; otherwise pick the key that's set;
    if both are set, default to Groq."""
...
def _build_messages(
    ...
    """Assemble the OpenAI-style message list Groq consumes directly and
    Gemini adapts internally."""
    context_text = (
        "\n\n---\n\n".join(context_chunks)
        if context_chunks
...
def generate_answer(
    ...
    """Public entrypoint. Deterministic no-context short-circuit avoids paying for a
    guaranteed-bad LLM call; all provider errors are sanitised to safe text."""
